...
namespace Telephone_DS //Telephone写的DataStructure的命名空间
{
    namespace arrayBase //Telephone写的基于数组的DataStructure的命名空间
    {
        namespace ArrayStorage //Telephone写的ArrayStorage的命名空间
        {
            ...
            public:
                ...
                ArrayStorage(ArrayStorage<T>& src)    //拷贝构造函数
                {
                    scale = src.scale;
                ...
                ArrayStorage<T> &operator=(ArrayStorage<T>& right)
                {
                    if(this != &right)
                    {
                        /******释放******/
                ...
                void edit(long index , T x)
                {
                    if(index >= 0 && index < length)
                    {
                        long memIndex = head - memHead;
                        memIndex += index;
                        *(memHead + (memIndex % memLen)) = x;
                        return;
                    }
                    char exp[100];
                    std::sprintf(exp , "[function edit()] Do not have such node : index(%ld)" , index);
                    throw std::out_of_range(exp);
                }
                void deleteFrom(long index , long num = 1)      // the index node itself will be deleted too（include）
                {
                    char exp[100];
                    if(index >= 0 && index < length)
                    {
